([インストール方法](#mocapforallのインストール方法) と [使い方１：モーションキャプチャするための準備編](#使い方１モーションキャプチャするための準備編) の部分をカバーするクイックスタートガイド)  
...
- [使い方１：モーションキャプチャするための準備編](#使い方１モーションキャプチャするための準備編)

  - [カメラの用意](#カメラの用意)
  - [カメラ校正](#カメラ校正)
  - [アプリ上での調整](#アプリ上での調整)

- [使い方２：キャプチャしたモーションの利用編](#使い方２キャプチャしたモーションの利用編)
  ...
  - [共有メモリへの書き出し](#共有メモリへの書き出し)
  
- [使い方３：その他の設定編](#使い方３その他の設定編)
...
## ダウンロード方法
### 無料試用版
[BOOTH](https://akiya-souken.booth.pm/items/3026474)および[Steam](https://store.steampowered.com/app/1759710/MocapForAll/)にて無料体験版を配布しています。  
...
#### [Steam](https://store.steampowered.com/app/1759710/MocapForAll/)での購入
購入前に、無料試用版でソフトウェアがお客様の環境で問題なく動作することを必ずご確認ください。  
また、利用規約を必ずご確認ください。  
https://store.steampowered.com//eula/1759710_eula_0

## BOOTHからのインストール方法
[手動でのインストール](#手動でのインストール)と、[ネットーワークインストーラでのインストール](#ネットーワークインストーラでのインストール)の二つの方法があります。
...
RTX30**シリーズとそれ以外で、下記の通り、使用するcuDNNとTensorRTのバージョンが異なることにご注意ください（※3）。   

RTX2080Tiを使っている場合は、cuDNN v8.0.5を使う必要があるかもしれません。下記の組み合わせをいろいろ試してみてください。  

なお、RTX30**シリーズについては、RTX3060Ti と RTX3070 でのみ動作を確認しています。それ以外での動作は未検証です。（動いたよ！という方はぜひご一報ください）
...
> ※3： RTX30**シリーズ向けのバージョンは、[漆原 鎌足](https://twitter.com/kamatari_san/status/1435536643901902852)さまから情報提供いただきました。ありがとうございます！
...
## Steamからのインストールおよびアップデート方法
通常のゲーム同様に、Steamクライアントアプリを使用してインストールおよび更新するだけです。
すべての[付録](#付録省略可)が自動的にインストールされます。
TensorRTによるGPUアクセラレーションを使用するには、手動で[CUDA、cuDNN、およびTensorRTをインストール](#tensorrtのインストール)する必要があります。
...
3. Diamondクラスターを使うやり方  
    [このzip](https://github.com/Akiya-Research-Institute/MocapForAll-Wiki/raw/main/resources/calibration/DiamondMarkers.zip)にある"diamondMarker0.png"およびそれ以降を使います。「1.ChArUcoボードを使うやり方」と同様に、A2以上のサイズで印刷してください。 ![Camera-Calibration-Extrinsic-Marker-Diamond]({{ site.url }}{{ site.baseurl }}/assets/images/Camera-Calibration-Extrinsic-Marker-Diamond.jpg)
4. 人の動きを使うやり方  
    特に準備するものはありません。
...
これ以降の操作を行う前提として、MocapForAll上で視点を移動するには、WASDキーとマウスのドラッグを使用します。
また、画面の最大化は、F11キーでオンオフできます。
...
- 「Settings > Data export > Shared Memory > Export to shared memory」を有効にします。
...
<img src="{{ site.url }}{{ site.baseurl }}/assets/images/DataFormat-SharedMemory-Hand.png" alt="DataFormat-SharedMemory-Hand" style="zoom:60%;" /> 
...
共有メモリに書き込まれたデータをUnityで読み取りGameObjectに反映するサンプルプロジェクトを[こちら](https://github.com/Akiya-Research-Institute/MocapForAll-SharedMemory-Plugin-for-Unity)で公開しています。
...
v1.13から、下記の語句を引数に指定することで、起動時に特定の動作を実行させることができます。  
...
フランス語へ翻訳してくれた Gonzales kévin, "Raybox41130" 様に心から感謝します。
